...
" 2>/dev/null)
    ...
    if [ "$STATUS" = "completed" ]; then
        ...
        if command -v python3 >/dev/null 2>&1; then
            VIDEO_URLS=$(echo "$STATUS_RESPONSE" | python3 -c "
import sys, json
try:
    data = json.load(sys.stdin)
    if 'result' in data and 'video_urls' in data['result'] and len(data['result']['video_urls']) > 0:
        print(data['result']['video_urls'][0]['url'])
    else:
        sys.exit(1)
except:
    sys.exit(1)
" 2>/dev/null)
        ...
        if [ -z "$VIDEO_URLS" ]; then
            VIDEO_URLS=$(echo "$STATUS_RESPONSE" | tr -d '\n' | grep -o '"/output/[^"]*"' | head -1 | tr -d '"')
        fi
        
        if [ -z "$VIDEO_URLS" ]; then
            echo "警告: 无法从响应中提取视频URL"
            echo "完整响应: $STATUS_RESPONSE"
            exit 1
        fi
        
        # 构建完整URL
        FULL_URL="$BASE_URL$VIDEO_URLS"
        FILENAME=$(basename "$VIDEO_URLS")
